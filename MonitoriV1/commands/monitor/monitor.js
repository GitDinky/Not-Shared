...
module.exports = {
  ...
  run: async (client, message, args ) => {
	if (!args[0]) {
      return send("Please give a website link to monitor", message, "RED");
    }
		if (!isURL(args[0])) {
      return send(
        "Url is invalid, Make sure you send working URL",
        message,
        "RED"
      );
    }
...
// Coded By SLote & Snzy
    ...
    if (check) {
      if (check.link.length === 20) {
        return send(
          "You reached your limit, you can not add more than 20 websites.",
          message,
          "YELLOW"
        );
      }
    ...
    send("Website successfully added to monitoring!✅", message, "BLACK");
